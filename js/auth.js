...
window.API_BASE = 'https://hostel-grievance-portal.onrender.com/api/grievances';
const API_BASE = 'https://hostel-grievance-portal.onrender.com/api/grievances';

// Prevent any future changes to API_BASE
Object.defineProperty(window, 'API_BASE', {
  value: 'https://hostel-grievance-portal.onrender.com/api/grievances',
  writable: false,
  configurable: false
});
...
async function testConnection() {
  try {
    ...
    const FORCED_API_BASE = 'https://hostel-grievance-portal.onrender.com/api/grievances';
...
async function apiGet(endpoint) {
  ...
  // AGGRESSIVE TOKEN CHECK - NO API CALLS WITHOUT VALID TOKEN
  const token = localStorage.getItem('token');
  const user = localStorage.getItem('user');
  
  if (!token || !user) {
    console.error('🚨 CRITICAL: No token or user found!');
    console.error('🚨 Clearing storage and redirecting to login...');
    localStorage.clear();
    sessionStorage.clear();
    window.location.href = 'index.html';
    return null;
  ...
  // Check token format
  if (!token.startsWith('eyJ')) {
    console.error('🚨 CRITICAL: Invalid token format!');
    console.error('🚨 Clearing storage and redirecting to login...');
    localStorage.clear();
    sessionStorage.clear();
    window.location.href = 'index.html';
  ...
  try {
    const userData = JSON.parse(user);
    if (!userData.role) {
      console.error('🚨 CRITICAL: Invalid user data!');
      console.error('🚨 Clearing storage and redirecting to login...');
      localStorage.clear();
      sessionStorage.clear();
      window.location.href = 'index.html';
    ...
    console.error('🚨 CRITICAL: Corrupted user data!');
    console.error('🚨 Clearing storage and redirecting to login...');
    localStorage.clear();
    sessionStorage.clear();
    window.location.href = 'index.html';
